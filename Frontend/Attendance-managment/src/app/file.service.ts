...
export class FileService {
 ...
 private fileuserList: string[] = new Array<string>();
 private fileuserList$: Subject<string[]> = new ReplaySubject<string[]>(1);
...
constructor(
 ...
 public upload_user(fileName: string, fileContent: string): void {
   this.displayLoader$.next(true);
   this.http.put('/user_upload', {name: fileName, content: fileContent})
   .pipe(finalize(() => this.displayLoader$.next(false)))
   .subscribe(res => {
     this.fileuserList.push(fileName);
     this.fileuserList$.next(this.fileuserList);
   }, error => {
     this.displayLoader$.next(false);
   });
 }
